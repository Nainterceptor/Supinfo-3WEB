(function () {
    ...
    var form = document.getElementById('new-message-form'),
        $chatroom = jQuery('#chatroom'),
        $messageTemplate = jQuery('#message-template').removeAttr('id'),
        messageTemplate = $chatroom.html(),
        $statusEl = jQuery('#status'),
        $sayItButton = jQuery('#say-it-button'),
        $msgField = jQuery('#msg-field');
    ...
    function renderMessage(message, isUnsent) {
        ...
        var $msgEl = jQuery($.Mustache.render('message-template', data));
        if (isUnsent)
